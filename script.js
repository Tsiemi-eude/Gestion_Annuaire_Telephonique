/* Annuaire Téléphonique – affichage en TABLEAU
   - localStorage, import/export JSON, upload image base64
...
(() => {
  ...
  const initialsDataURL = (firstName, lastName) => {
    ...
    ctx.fillStyle = '#2C3E50'; ctx.font = 'bold 80px sans-serif';
  ...
  function load(){
    try{
      ...
      err('Lecture storage', e); contacts = [];
    }
  }
  function save(){
    try{
  ...
  function render(filter=''){
    ...
    if (list.length === 0){
      ...
      td.textContent = 'Aucun contact. Utilisez “Ajouter”.';
      tr.appendChild(td);
      E.contactsTbody.appendChild(tr);
      return;
    }
    ...
    list.forEach(c => {
      ...
      img.className = 'thumb';
      img.alt = 'Photo';
      img.src = c.photo || initialsDataURL(c.firstName, c.lastName);
      tdPhoto.appendChild(img);

      // Nom
      const tdLast = document.createElement('td');
      tdLast.textContent = c.lastName || '';

      // Prénom
      const tdFirst = document.createElement('td');
      tdFirst.textContent = c.firstName || '';
      ...
      btnDel.type = 'button';
      btnDel.className = 'btn danger btn-delete';
      btnDel.style.marginLeft = '8px';
      btnDel.textContent = 'Supprimer';

      tdActions.append(btnEdit, btnDel);

      tr.append(tdPhoto, tdLast, tdFirst, tdPhone, tdEmail, tdAddr, tdActions);
      frag.appendChild(tr);
    });
    E.contactsTbody.appendChild(frag);
  }
  ...
  async function exportContacts(){
    ...
    }catch(e){ err(e); alert('Impossible d’exporter les contacts.'); }
  }
  async function importContacts(file, replace=true){
    ...
    }catch(e){ err(e); alert('Erreur import (format/lecture).'); }
  }
  ...
  function bindEvents(){
    ...
    // Header
    E.btnAdd?.addEventListener('click', () => { resetForm(); openModal('Ajouter un contact'); });
    ...
    E.importFile?.addEventListener('change', async (e) => {
      ...
      await importContacts(f, replace);
      e.target.value = '';
    });
    ...
    E.photoInput?.addEventListener('change', async (e) => {
      ...
      try{
        ...
        E.photoPreview.hidden = false;
        E.clearPhoto.hidden = false;
      }catch(e2){ err(e2); alert('Impossible de charger l’image.'); }
    });
    ...
    E.contactForm?.addEventListener('submit', async (e) => {
      ...
      if (!E.firstName.value.trim() && !E.lastName.value.trim() && !E.phone.value.trim()){
        alert('Au moins un nom/prénom ou un téléphone.'); return;
      }
    ...
    // Délégation d’événements sur le TBODY pour Edit/Suppr
    E.contactsTbody.addEventListener('click', (e) => {
      ...
      if (e.target.closest('.btn-edit')){
        fillForm(contact); openModal('Modifier le contact');
        return;
      }
      if (e.target.closest('.btn-delete')){
        const label = `${contact.firstName||''} ${contact.lastName||''}`.trim() || contact.phone || 'ce contact';
  ...
  function init(){
    try{
      cacheElements(); load(); seedIfEmpty(); bindEvents(); render();
      log('Application initialisée ✅');
    }catch(e){
      err('Init failed', e);
      alert('Une erreur empêche l’interface d’être interactive. Ouvre la console (F12).');
    }
  }
